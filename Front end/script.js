...
// ─── THEME ─────
function loadTheme() {
  const theme = localStorage.getItem('me2u-theme') || 'dark';
  currentTheme = theme;
  if (theme === 'light') {
    ...
    if (lightLabel) {
      ...
      document.getElementById('theme-dark-label').classList.remove('active');
    }
  } else {
    ...
    if (darkLabel) {
      ...
      document.getElementById('theme-light-label').classList.remove('active');
    }
  }
}

window.setTheme = function(theme) {
  localStorage.setItem('me2u-theme', theme);
  currentTheme = theme;
  if (theme === 'light') {
    document.documentElement.classList.add('light-mode');
    const lightLabel = document.getElementById('theme-light-label');
...
document.addEventListener('click', function(e) {
  const settingsBtn = document.querySelector('.hbtn.settings');
  const settingsModal = document.getElementById('settings-modal');
  if (settingsBtn && settingsModal && !settingsBtn.contains(e.target) && !settingsModal.contains(e.target)) {
...
window.addEventListener('load', function() {
  // Reapply theme after all styles load
  const theme = localStorage.getItem('me2u-theme') || 'dark';
  if (theme === 'light') {
    document.documentElement.classList.add('light-mode');
  }
